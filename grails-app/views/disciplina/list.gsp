			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="dia" title="${message(code: 'disciplina.dia.label', default: 'Dia')}" />
					
						<g:sortableColumn property="horario" title="${message(code: 'disciplina.horario.label', default: 'Horario')}" />
				...
				<g:each in="${disciplinaInstanceList}" status="i" var="disciplinaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: disciplinaInstance, field: "dia")}</td>
					
						<td>${fieldValue(bean: disciplinaInstance, field: "horario")}</td>
